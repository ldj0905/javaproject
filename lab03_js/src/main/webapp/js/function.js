 ...
 * 파라미터를 선언할 때 cosnt, let, var 키워드를 사용하지 않음.
 * 
 *
 */
...
function testArgss(x, y){
    return x - y;
    
}
console.log(testArgss(4,2));
...
result = multiply(2,3); //아규먼트 y를 전달 하면 기본값은 무시 .
console.log(result);

result = multiply(2); //아규먼트 y를 전달 하지 않으면 기본값이 사용된다 .
console.log(result);
...
/*
 ...
 2. 함수 자체를 변수에 저장할 수 있음.
 3. 다른 함수를 호출할 때 아규먼트로 함수 (객체)를 전달할 수 있음.
 4. 함수 내부에서 다른 함수를 선언할 수 있음. 
 5. 함수 (객체)를 리턴할 수 있음.
*/

const plus = add; //함수 객체 add를 변수 plus 에 할당(저장).
console.log(plus);
console.log(plus());

//익명 함수 (annoymous function): 이름이 없는 함수.
...
//-> 이벤트 리스너(핸들러) 설정 할 때 많이 사용되는 코드 패턴.

//콜백(callback): (나중에 호출하기 위해서) 아규먼트로 전달 하는 함수.
