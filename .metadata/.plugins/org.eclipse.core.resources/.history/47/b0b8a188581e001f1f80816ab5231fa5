...
public class PostUpdateController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		int id = Integer.parseInt(req.getParameter("id")); //name="id"였어서
		...
		// TODO: 서비스 계층의 메서드를 호출해서 포스트 업데이트.
		postService.update(post);
		// 상세보기 화면으로 이동(redirect)
		String url = req.getContextPath() + "/post/details?id=" + id;
		log.debug("redirect: {}",url); // 그냥 로그 출력해보기
		resp.sendRedirect(url); //url로 보내버리기
	}
