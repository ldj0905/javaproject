 ...
 */
//모든 javaScript에 들어간다고 생각하면됨
document.addEventListener('DOMContentLoaded', () => {
    // btnToggleComment 버튼 요소를 찾음.
    const btnToggleComment = document.querySelector('button#btnToggleComment'); //querySelector는 객체를 한개. 리턴해줌
    //querySelectAll >>> 객체 배열을 리턴해준다.
    ...
    function makeCommentElements(data) {
        ...
        const btnDeletes = document.querySelectorAll('button.btnDeleteComment'); //윗쪽에서 버튼을 만들때 준 클래스이름과 동일 
    ...
    function deleteComment(event) {
        ...
        //Ajax 요청을 보낼 URI
        const uri = `../api/comment/${id}`; //삭제 요청을 보낼 주소
        //Ajax 요청을 보냄.
        axios
        .delete(uri)
        .then((response) => {
