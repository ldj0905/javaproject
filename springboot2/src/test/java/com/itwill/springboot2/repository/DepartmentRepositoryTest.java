...
@Slf4j
@TestMethodOrder(MethodOrderer.OrderAnnotation.class) //테스트 메서드의 순서를 지정하겠다. 
//여기서는 메서드 순서를 지정하는 객체 오더 에너테이션으로 하겠다.
@SpringBootTest
...
//    @Test
    @Order(1) //테스트 첫번째 순서. 성공하면 order(2)로 넘어감.
    public void test() {
    ...
    @Transactional
//    @Test
    @Order(2) //테스트 두번째 순서. 성공하면 order(3)로 넘어감.
